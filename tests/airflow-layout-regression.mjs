...
assert.ok(main.includes('const splitPortFour = isFsVsHciModel() && !sameSide;'));
...
assert.ok(main.includes('rear: code === "HH"'));
assert.ok(main.includes('data-rear="${placement.rear}"'));
assert.ok(main.includes('const showStRearSideKey = stFlowLayout && stHasRearPort(draft!.layoutCode);'));
assert.ok(main.includes('class="st-rear-side-key"'));
assert.ok(main.includes('draft!.installationMode === "floor" && !stFlowLayout'));
